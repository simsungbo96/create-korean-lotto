...
class MainActivity : AppCompatActivity() {
    ...
    private fun initRunButton(){
        ...
        list.forEachIndexed { index, number ->  /*이떄 number은 list[index]의 값을 뜻함.*/
            ...
            if(index == 6){
                textView.text = number.toString()
                textView.isVisible =true
                textView.background =ContextCompat.getDrawable(this@MainActivity,R.drawable.circle_pupple)
    ...
    private fun getRandomNumber(): List<Int> {
        ...
        return (pickNumberSet.toList() + numberList.subList(0, 6 - pickNumberSet.size)).sorted() + numberList[7]
    }
